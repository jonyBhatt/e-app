...
export const PwaVoterSearch: React.FC = () => {
  ...
  const filteredPeople = useMemo(() => {
    return people.filter((person) => {
      const matchesSearch =
        ...
        person.voter_no?.includes(searchTerm);
      const matchesJob =
        filters.job === "" || person.job?.includes(filters.job);
      const matchesWord =
        filters.word === "" || person.word_no === filters.word;
      const matchesDob =
        filters.dob === "" || person.date_of_birth === filters.dob;
  ...
  if (view === "list")
    return <SelectedListPage onBack={() => setView("search")} />;
        ...
        <div className="rounded-2xl border bg-white p-5 shadow-sm">
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {filters.job && (
                <Badge
                  label={`পেশা: ${filters.job}`}
                  onRemove={() => setFilters({ ...filters, job: "" })}
                />
              )}
              {filters.word && (
                <Badge
                  label={`ওয়ার্ড: ${filters.word}`}
                  onRemove={() => setFilters({ ...filters, word: "" })}
                />
              )}
              {filters.dob && (
                <Badge
                  label={`জন্ম: ${filters.dob}`}
                  onRemove={() => setFilters({ ...filters, dob: "" })}
                />
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col items-start gap-3">
          <div className="mt-8 flex items-center justify-between px-2 w-full">
            <label className="flex cursor-pointer items-center gap-3">
              <div
                onClick={() =>
                  isAllSelectedOnPage ? clearAll() : selectMany(currentData)
                }
                className={cn(
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border bg-white shadow-sm">
          ...
          <div className="bg-[#F8FAFC] p-4">
            <p className="mb-4 text-center text-sm text-[#64748B]">
              প্রদর্শন করা হচ্ছে {startIndex + 1} থেকে{" "}
              {Math.min(startIndex + itemsPerPage, filteredPeople.length)} (মোট{" "}
